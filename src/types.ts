 ...
 */
export interface FastaSequence extends Sequence {
 ...
 * Format: @id description\nsequence\n+\nquality
 */
export interface FastqSequence extends Sequence {
  readonly format: 'fastq';
  /** Quality scores as ASCII string */
  readonly quality: string;
  /** Quality encoding system detected or specified */
...
export const GenomicRange = type({
  ...
  contains: (pos: number) => pos >= range.start && pos < range.end,
  overlaps: (other: typeof range) =>
    range.chromosome === other.chromosome && range.start < other.end && range.end > other.start,
  toString: () => `${range.chromosome}:${range.start}-${range.end}`,
}));
