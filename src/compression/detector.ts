 ...
 * ```typescript
 * const format = fromExtension('/data/genome.fasta.gz');
...
export function fromExtension(filePath: string): CompressionFormat {
  ...
  try {
    ...
    const gzipExtensions = [...COMPRESSION_EXTENSIONS.gzip, ...COMPRESSION_EXTENSIONS.genomic_gzip];
    ...
    const zstdExtensions = [...COMPRESSION_EXTENSIONS.zstd, ...COMPRESSION_EXTENSIONS.genomic_zstd];
 ...
 * const detection = fromMagicBytes(bytes);
 ...
 * const detection = await fromStream(stream);
...
export async function fromStream(
  ...
  try {
    ...
    return fromMagicBytes(headerBytes);
  } catch (error) {
 ...
 *
 * @example Hybrid detection with magic bytes
 * ```typescript
 * const detection = hybrid('/data/genome.fasta.gz', magicBytes);
 * if (detection.confidence > 0.9) {
 *   console.log('High confidence hybrid detection:', detection.format);
 * }
 * ```
 *
 * Tiger Style: Function must not exceed 70 lines, minimum 2 assertions
...
export function hybrid(filePath: string, bytes?: Uint8Array): CompressionDetection {
  ...
  try {
    const extensionFormat = fromExtension(filePath);
    ...
    const magicDetection = fromMagicBytes(bytes);
 ...
 * @throws {CompressionError} If detection result validation fails
 *
 * @example Validate detection confidence
 * ```typescript
 * const detection = fromMagicBytes(bytes);
 * if (isReliable(detection)) {
 *   console.log('Reliable detection:', detection.format);
 * }
 * ```
 */
