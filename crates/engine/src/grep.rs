...
//! `find_pattern_batch` engine functions. It wraps sassy's `Searcher` to
...
pub type MatchTriple = (u32, u32, u32);
pub type PerSeqMatches = SmallVec<[MatchTriple; 4]>;

/// How the kernel should match the pattern against sequences.
///
/// This enum makes invalid flag combinations unrepresentable. The adapter
/// layer converts the JS-friendly `(case_insensitive, search_both_strands)`
...
impl SearchMode {
    pub fn from_flags(case_insensitive: bool, search_both_strands: bool) -> Self {
...
impl SearchContext {
    ...
    pub fn new(pattern: &[u8], max_edits: u32, mode: &SearchMode) -> Self {
    ...
    pub fn new_with_positions(pattern: &[u8], max_edits: u32, case_insensitive: bool) -> Self {
    ...
    pub fn contains_match(&mut self, seq: &[u8]) -> bool {
        if seq.is_empty() {
            return false;
        }
        match self {
    ...
    pub fn find_matches(&mut self, seq: &[u8]) -> PerSeqMatches {
        match self {
            Self::Ascii {
                searcher,
                pattern,
