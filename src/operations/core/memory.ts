...
export class DefaultMemoryMonitor implements MemoryMonitor {
  ...
  constructor(thresholdBytes: number = 1_000_000_000) { // Default 1GB
...
export class ExternalSorter<T> {
   ...
   */
  private async *mergeSortedChunks(
    compareFn: (a: T, b: T) => number
  ): AsyncIterable<T> {
    ...
    const heap = new MinHeap<{ value: T; source: number }>(
      (a, b) => compareFn(a.value, b.value)
    );
  ...
  private async createFileIterator(fileName: string): Promise<AsyncIterator<T>> {
    ...
    return {
      async next(): Promise<IteratorResult<T>> {
        ...
        const value = line ? deserialize(line) : undefined;
...
class MinHeap<T> {
  ...
  private bubbleDown(index: number): void {
    ...
    while (true) {
      ...
      if (leftIndex < length && left !== undefined && smallest !== undefined &&
      ...
      if (rightIndex < length && right !== undefined && currentSmallest !== undefined &&
...
export class AdaptiveBuffer<T> {
  ...
  private shouldFlush(): boolean {
    return this.buffer.length >= this.maxSize || 
           this.monitor.shouldSwitchToExternal();
  }
