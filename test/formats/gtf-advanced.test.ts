/**
 * Comprehensive GTF Advanced Test Suite
 *
 * Implements advanced test requirements from GTF_DOMAIN_ANALYSIS_REPORT.md
 * targeting real-world edge cases, database format variations, and genomics
 * domain expertise for exceptional quality GTF parsing.
 *
 * Categories:
 * 1. Database Format Variations (GENCODE, Ensembl, RefSeq)
 * 2. Real-World Edge Cases (malformed syntax, recovery strategies)
 * 3. Feature Type Hierarchy Testing (gene models, relationships)
 * 4. Coordinate System Validation (1-based, boundary conditions)
 * 5. Performance and Memory Testing (large datasets, streaming)
 *
 * Design principles:
 * - Uses real GTF examples from domain research, not synthetic data
 * - Documents current implementation behavior, doesn't assume it's wrong
 * - Tests both positive (should work) and negative (should error) cases
 * - Follows existing test/formats/bed.test.ts patterns for consistency
 * - Comprehensive JSDoc on test intent and genomics context
 */
...
describe("GTF Query API - Advanced Feature Testing", () => {
  ...
  describe("Template Literal Biological Validation", () => {
    ...
    test("filterByFeature constrains to valid GTF feature types", async () => {
      ...
      const testFeatures = [
        { seqname: "chr1", feature: "gene", attributes: { gene_id: "ENSG001" } } as GtfFeature,
        {
          seqname: "chr1",
          feature: "transcript",
