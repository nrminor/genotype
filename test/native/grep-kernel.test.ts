...
function grepBatchFromStrings(
  ...
  return kernel.grepBatch(data, offsets, Buffer.from(pattern), maxEdits, caseInsensitive, searchBothStrands);
}
...
describeNative("grep native kernel (requires just build-native-dev)", () => {
  describe("raw FFI contract", () => {
    ...
    testNative("empty batch returns empty buffer", () => {
      const kernel = requireNativeKernel();
      const results = kernel.grepBatch(Buffer.alloc(0), new Uint32Array([0]), Buffer.from("GATC"), 0, false, false);
    ...
    testNative("throws on malformed offsets: out of bounds", () => {
      const kernel = requireNativeKernel();
      expect(() => {
        kernel.grepBatch(Buffer.from("ATCG"), new Uint32Array([0, 100]), Buffer.from("A"), 0, false, false);
    ...
    testNative("throws on malformed offsets: non-monotonic", () => {
      const kernel = requireNativeKernel();
      expect(() => {
        kernel.grepBatch(Buffer.from("ATCGATCG"), new Uint32Array([0, 4, 2, 8]), Buffer.from("A"), 0, false, false);
    ...
    for (const { label, ignoreCase, allowMismatches, searchBothStrands } of parityMatrix) {
      testNative(`parity (exact): ${label}`, () => {
        ...
        const nativeResults = grepBatchFromStrings(
          sequences, pattern, allowMismatches, ignoreCase, searchBothStrands,
        );

        for (let i = 0; i < sequences.length; i++) {
          const tsResult = tsFallbackMatch(sequences[i]!, pattern, allowMismatches, ignoreCase, searchBothStrands);
  ...
  describe("known parity divergence", () => {
    ...
    testNative("Iupac profile is case-insensitive even when ignoreCase=false with bothStrands", () => {
