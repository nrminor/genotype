...
import { StreamUtils } from '../../src/io/stream-utils';
...
describe('FileReader', () => {
  describe('File Existence Checking', () => {
    test('should detect existing files', async () => {
      expect(await FileReader.exists(TEST_FILES.small)).toBe(true);
      expect(await FileReader.exists(TEST_FILES.empty)).toBe(true);
    });

    test('should detect non-existing files', async () => {
      expect(await FileReader.exists(TEST_FILES.nonexistent)).toBe(false);
    });

    test('should handle invalid paths', async () => {
      await expect(FileReader.exists('')).rejects.toThrow(FileError);
      await expect(FileReader.exists('\0invalid')).rejects.toThrow(FileError);
    });

    test('should distinguish files from directories', async () => {
      expect(await FileReader.exists(TEST_FILES.directory)).toBe(false);
    });
  });

  describe('File Size Detection', () => {
    test('should get correct file sizes', async () => {
      expect(await FileReader.getSize(TEST_FILES.small)).toBe(13);
      expect(await FileReader.getSize(TEST_FILES.empty)).toBe(0);
      expect(await FileReader.getSize(TEST_FILES.large)).toBe(100000);
    });

    test('should throw for non-existing files', async () => {
      await expect(FileReader.getSize(TEST_FILES.nonexistent)).rejects.toThrow(FileError);
    });
  });

  describe('File Metadata', () => {
    test('should get comprehensive metadata', async () => {
      const metadata = await FileReader.getMetadata(TEST_FILES.small);
    ...
    test('should handle UTF-8 files correctly', async () => {
      const metadata = await FileReader.getMetadata(TEST_FILES.utf8);
      expect(metadata.readable).toBe(true);
  ...
  describe('Stream Creation', () => {
    test('should create readable stream for valid files', async () => {
      const stream = await FileReader.createStream(TEST_FILES.small);
    ...
    test('should handle empty files', async () => {
      const stream = await FileReader.createStream(TEST_FILES.empty);
      const reader = stream.getReader();
    ...
    test('should respect buffer size options', async () => {
      const stream = await FileReader.createStream(TEST_FILES.medium, {
        bufferSize: 100,
      });
    ...
    test('should throw for non-existing files', async () => {
      await expect(FileReader.createStream(TEST_FILES.nonexistent)).rejects.toThrow(FileError);
    });
  ...
  describe('String Reading', () => {
    test('should read small files to string', async () => {
      const content = await FileReader.readToString(TEST_FILES.small);
      expect(content).toBe('Hello, World!');
    });

    test('should handle empty files', async () => {
      const content = await FileReader.readToString(TEST_FILES.empty);
      expect(content).toBe('');
    });

    test('should handle UTF-8 content', async () => {
      const content = await FileReader.readToString(TEST_FILES.utf8);
      expect(content).toBe('Hello, 世界! 🌍');
    });

    test('should respect file size limits', async () => {
      await expect(
        FileReader.readToString(TEST_FILES.large, {
          maxFileSize: 1000,
        })
    ...
    test('should handle different encodings', async () => {
      const content = await FileReader.readToString(TEST_FILES.binary, {
        encoding: 'binary',
      });
  ...
  describe('Error Handling', () => {
    test('should provide detailed error context', async () => {
      try {
        await FileReader.getSize(TEST_FILES.nonexistent);
    ...
    test('should handle timeout errors', async () => {
      ...
      try {
        await FileReader.readToString(TEST_FILES.large, {
...
describe('StreamUtils', () => {
  describe('Line Reading', () => {
    test('should read lines from stream', async () => {
      const stream = await FileReader.createStream(join(FIXTURES_DIR, 'lines.txt'));
      const lines: string[] = [];
    ...
    test('should handle mixed line endings', async () => {
      const stream = await FileReader.createStream(join(FIXTURES_DIR, 'mixed-endings.txt'));
      const lines: string[] = [];
    ...
    test('should handle very long lines', async () => {
      const stream = await FileReader.createStream(join(FIXTURES_DIR, 'long-lines.txt'));
      const lines: string[] = [];
  ...
  describe('Statistics and Monitoring', () => {
    test('should provide stream processing statistics', async () => {
      const stream = await FileReader.createStream(join(FIXTURES_DIR, 'medium.txt'));
...
describe('Integration Tests', () => {
  ...
  test('should handle concurrent file operations', async () => {
    const promises = Array.from({ length: 5 }, (_, i) => FileReader.readToString(TEST_FILES.small));
