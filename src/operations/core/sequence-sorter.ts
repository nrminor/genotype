...
import type { Sequence, FastqSequence } from '../../types';
...
export class SequenceSorter {
  ...
  async *sort(sequences: AsyncIterable<Sequence>): AsyncGenerator<Sequence> {
    ...
    const input = this.options.unique 
      ? this.deduplicate(sequences)
      : sequences;
  ...
  async sortInMemory(sequences: AsyncIterable<Sequence>): Promise<Sequence[]> {
    ...
    for await (const seq of sequences) {
      if (!this.options.unique || !this.seenIds?.has(this.getSequenceKey(seq))) {
        array.push(seq);
        if (this.options.unique) {
  ...
  async *getTopN(
    ...
    const invertedCompareFn = (a: Sequence, b: Sequence) => -this.compareFn(a, b);
...
class MinHeap<T> {
  ...
  private bubbleDown(index: number): void {
    ...
    while (true) {
      ...
      if (leftIndex < length && left !== undefined && current !== undefined &&
          this.compareFn(left, current) < 0) {
        smallestIndex = leftIndex;
      }
      
      const smallest = this.heap[smallestIndex];
      if (rightIndex < length && right !== undefined && smallest !== undefined &&
