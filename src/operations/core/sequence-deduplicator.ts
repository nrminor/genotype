...
export class SequenceDeduplicator {
  ...
  getStats(): DeduplicationStats {
    ...
    if ('estimatedFPR' in bloomStats) {
      ...
      const avgFPR = bloomStats.filters.reduce((sum, f) => sum + f.estimatedFPR, 0) / bloomStats.filters.length;
...
export class ExactDeduplicator {
  ...
  constructor(
    ...
    if (typeof strategy === 'function') {
      ...
      switch (strategy) {
        case 'sequence':
          this.getKey = (seq) => processSequence(seq.sequence);
          break;
        case 'id':
          this.getKey = (seq) => seq.id;
          break;
        case 'both':
          this.getKey = (seq) => `${seq.id}:${processSequence(seq.sequence)}`;
          break;
        case 'exact':
          this.getKey = (seq) => JSON.stringify({
            id: seq.id,
            sequence: processSequence(seq.sequence),
            description: seq.description
          });
